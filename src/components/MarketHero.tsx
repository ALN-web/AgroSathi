...
import { Cpu, TrendingUp, Clock, Target, LineChart } from 'lucide-react';
      ...
      <div className="relative z-20 max-w-[1500px] mx-auto px-6 w-full flex flex-col xl:flex-row items-start xl:items-end justify-between gap-12">
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-2 bg-[#134E36]/80 backdrop-blur-md border border-[#B8F24A]/30 text-[#B8F24A] font-semibold text-sm rounded-full mb-6 shadow-lg">
            <LineChart className="w-4 h-4" /> Market Price Prediction
          </span>
          ...
          <h2 className="text-[2.5rem] md:text-[3rem] font-bold text-[#B8F24A] leading-[1.1] tracking-tight mb-6">
            For Better Selling Decisions
          </h2>
          <p className="text-lg text-white/80 leading-relaxed mb-8 max-w-2xl drop-shadow-md">
            Our AI model analyzes historical data, market trends, demand-supply, weather patterns and seasonality to predict future prices and help you sell at the right time for maximum profit.
          </p>
          
          {/* Feature Pills */}
          <div className="flex flex-wrap gap-4">
            <FeaturePill icon={<Cpu className="w-5 h-5" />} title="AI Powered" subtitle="ML Prediction" />
            <FeaturePill icon={<Target className="w-5 h-5 text-[#B8F24A]" />} title="95%+" subtitle="Prediction Accuracy" />
            <FeaturePill icon={<Clock className="w-5 h-5" />} title="Real-time" subtitle="Market Analysis" />
            <FeaturePill icon={<TrendingUp className="w-5 h-5" />} title="Profit" subtitle="Maximization" />
          </div>
        </motion.div>
      </div>
    </div>
